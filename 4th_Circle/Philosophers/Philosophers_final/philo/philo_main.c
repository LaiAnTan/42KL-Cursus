...
/*   Updated: 2023/01/19 17:40:52 by tlai-an          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "philo.h"

static void	delay_odd(t_data *data, int philo_num)
{
...
int	thread_func(void *args)
{
	...
	unlock_forks(data, curr_thread_index);
	return (0);
}
...
int	main(int argc, char **argv)
{
	...
	if (init_struct(&data, argc, argv) == -1)
		return (error(&data));
	init_mutex(&data);
	if (create_all_threads(&data, (void *) thread_func, (void *) &data))
		return (error(&data));
	pthread_mutex_lock(&data.start_mtx);
	data.start = TRUE;
	pthread_mutex_unlock(&data.start_mtx);
